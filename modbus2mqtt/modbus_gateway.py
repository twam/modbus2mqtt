        ...
        try:
            try:
                async with AsyncModbusTcpClient(
                    ...
                    if client.connected:
                        if config.get("devices", None) is None:
                            logging.info(f"No devices defined for gateway {name}.")
                            return

                        async with asyncio.TaskGroup() as tg:
                            for unit, device_config in config.get("devices", None).items():
                                try:
                                    module = importlib.import_module(f".devices.{device_config['class']}", __package__)
                                except ModuleNotFoundError:
                                    raise InvalidConfigurationException(f"Class '{device_config['class']}' not supported.")
